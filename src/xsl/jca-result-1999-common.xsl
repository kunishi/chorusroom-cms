...
<!-- $Id: jca-result-1999-common.xsl,v 1.14 2000/10/03 11:34:59 kunishi Exp $ -->
...
body { background-image: url(%%IMAGEDIR%%/background.png); }
  ...
  <xsl:template name="encodinglink">
	...
	<xsl:element name="a">
	  <xsl:attribute name="href">
	    <xsl:value-of select="concat(@出力, '.html')" />
	  </xsl:attribute>
	  <xsl:text>JISページ</xsl:text>
	</xsl:element>
      </xsl:if>
      <xsl:if test="$suffix='.html'">
	<xsl:element name="a">
	  <xsl:attribute name="href">
	    <xsl:value-of select="concat(@出力, '.utfhtml')"/>
	  </xsl:attribute>
